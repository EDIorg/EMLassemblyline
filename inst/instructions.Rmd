---
title: "Instructions for EMLassemblyline"
author: "Colin Smith"
...
Here is a decision tree for the assembly line to help you to decide which functions you should call to maximize the metadata for your dataset. Each step will ask you to fill out a series of templates (metadata parts) that are later rendered into EML metadata by the `make_eml` function.

# Step 1: Create a working directory for your dataset

Create a new directory for your dataset. This is where the metadata parts created in the assembly line process will be stored, and available to you in the future should you need to change the metadata content.

Name this directory after your dataset. Span spaces with underscores (e.g. "gleon_chloride")

# Step 2: Move your dataset to this directory

Move the final versions of your data entities (tables or zip files) into this directory. These should be the final versions of the data you are ready to publish. Do not include draft files or parts of the dataset you will not be publishing.

Rename these files following these rules: (1) replace blank spaces with underscores (2) remove symbols, parentheses, and periods.

# Step 3: Select an intellectual rights license

We recommend two intellectual rights licenses to be included with your dataset: (1) "CC0" the most accomodating of data reuse and (2) "CCBY" requires attribution. We do allow a 3rd option "other" in which you can specify a customized intellectual rights license but advise against this.
...
# Step 5: Identfy the types of data in your dataset

The assembly line works for many types of data but you will need to tell it what kind you have. There are 4 types to select from: (1) "table" (2) "spatial_vector" (3) "spatial_raster" (4) "other".

### table

A flat file composed of columns containing variables and rows containing observations. Column names must follow these rules: (1) replace blank spaces with underscores (2) remove symbols, parentheses, and periods.

### spatial_vector

A zip directory containing spatial vector files and supporting files. Often used within the geospatial domain.

### spatial_raster

A zip directory containing spatial raster files and supporting files. Often used within the geospatial domain.

### other

Other data types not included in the above categories. These data should be aggregated in a zip directory.

# Step 6: Import the core metadata templates

Run the function `import_templates` to populate the working directory with the core metadata templates for you to complete. You will need to supply a few arguments for this function to work correctly. When this function runs it will perform a series of checks and report any issues found.
...
import_templates(path = "/Users/csmith/Desktop/gleon_chloride",
                 license = "CC0",
                 data.names = c("lake_concentrations.csv",
