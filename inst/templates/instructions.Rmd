...
A tab delimited __catvars_datatablename.txt__ will be created for each of your data tables containing categorical variables. Open these in a spreadsheet editor and add definitions for each code. Blank codes are not valid. Replace these with character strings and rerun `define_catvars`.

# Step 17: Geographic coverage

If your dataset contains more than one sampling location (point locations are currently supported, areas are not), then you will want to add this information to your metadata. Often a data user will search for data withing a geographic area.
...
You may see a series of warnings reported after `make_eml` has completed. These can be ignored if your EML passes validation. If validation fails, open the EML file in an XML editor and look for the invalid section. Often a minor tweak to the EML can be made manually to bring it into compliance with the EML schema.


# Step 20: Upload your data package to the EDI repository
