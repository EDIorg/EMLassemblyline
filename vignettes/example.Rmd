---
title: "Making EML for the GLEON Lake Chloride dataset (an example)"
author: "Colin Smith"
date: "`r Sys.Date()`"
...
Here we demonstrate use of the EMLassemblyline R code package to create EML for the Global Lakes Ecological Observatory Network (GLEON) Lake Chloride dataset. The corresponding data package has been curated with the Environmental Data Initiative (EDI) and is publicly accessible through the EDI data portal of the PASTA repository [here](https://portal.edirepository.org/nis/mapbrowse?scope=edi&identifier=8).
...
Create a working directory for the dataset and name it using underscore characters "_" to span spaces between words (e.g. gleon_chloride). This directory stores metadata parts created in the assembly line and should be semi-permanent. These files will need to be edited in the event the data package changes and/or the EML needs revision.

![](dataset_working_directory.PNG)
...
Import metadata templates to the working directory. These metadata templates form the core of the metadata that will be later translated into EML. To import these templates run the import_templates.R function with a path to the working directory.
