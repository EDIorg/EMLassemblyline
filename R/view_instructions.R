...
#'     'gleon_chloride_lake_characteristics'.
#'          
...
#'     a tab delimited file that can be edited with a spreadsheet editor. Rename the file
#'     according to convention (e.g. gleon_chloride_personnel). Valid entries for role are: 
...
#'         character strings are present in an otherwise "numeric" attribute, this attribute must be 
#'         classified as "character". Similarly if any values of a "Date" attribute do not 
...
#'         provide units. If the attribute is numeric but does 
#'         not have units, enter "dimensionless". If the 
...
#'         cannot be converted to SI then list it in the attribute defintion
...
#'         leave this field blank. Below are rules for 
