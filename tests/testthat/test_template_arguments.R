...
testthat::test_that("Inputs = NULL", {
  ...
  expect_true(
    all(names(output$x) %in% c('template', 'data.table', 'spatial.raster', 'spatial.vector', 'other.entity', 'data.objects')))
...
testthat::test_that("Inputs = other entities", {
  ...
  for (i in 1:length(names(output$x$other.entity))) {
    expect_true(nrow(output$x$other.entity[[i]]$content) == 0)
  }
  unlink(paste0(tempdir(), "/pkg_260/data_objects"), force = T, recursive = T)
})
...
testthat::test_that("data.object facilitates general data object reads.", {
  ...
  expected_names <- c("template", "data.table", "spatial.raster", "spatial.vector", "other.entity", "data.objects")
