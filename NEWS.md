...
* __Enhancement:__ Release `create_directory.R`, a function to create a commonly used directory structure for EMLassemblyline inputs and outputs.
* __Bug fix:__ `import_templates.R` no longer fails to import template to empty directories.
* __Bug fix:__ `import_templates.R` can now be run on datasets that don't have data tables.
